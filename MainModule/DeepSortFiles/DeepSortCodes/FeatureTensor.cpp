	...
	: tf_model_meta(model_meta), tf_model_data(model_data)
{
	...
	if(status == false) 
	{
		std::cout << "FeatureTensor.cpp exiting" << std::endl;
		exit(1);
	}
}
...
void FeatureTensor::_init(const std::string& model_meta, const std::string& model_data)
{

	tf_model_meta = model_meta;
	tf_model_data = model_data;
	//prepare model:
	bool status = init();
	if(status == false) 
	{
